...
if [ ! -f "$HOME/.local/bin/shell-guardian" ]; then
    ...
    if [ -f "${GUARDIAN_DIR}/shell-guardian.bin" ]; then
        echo -e "${YELLOW}⚠️ Using pre-compiled guardian binary...${NC}"
        cp "${GUARDIAN_DIR}/shell-guardian.bin" "$HOME/.local/bin/shell-guardian"
        chmod +x "$HOME/.local/bin/shell-guardian"
    # Next try to build from source
    elif [ -f "${GUARDIAN_DIR}/shell-guardian.rs" ] && command -v rustc &>/dev/null; then
        echo -e "${YELLOW}⚠️ Building shell-guardian from source...${NC}"
        rustc -O "${GUARDIAN_DIR}/shell-guardian.rs" -o "$HOME/.local/bin/shell-guardian"
        chmod +x "$HOME/.local/bin/shell-guardian"
    # Finally use bash fallback
    else
        echo -e "${YELLOW}⚠️ Using bash fallback guardian...${NC}"
        cp "${GUARDIAN_DIR}/bash-guardian-fallback.sh" "$HOME/.local/bin/shell-guardian"
        chmod +x "$HOME/.local/bin/shell-guardian"
    fi
    
    # Always ensure the fallback is available directly
    cp "${GUARDIAN_DIR}/bash-guardian-fallback.sh" "$HOME/.local/bin/shell-guardian-fallback"
    chmod +x "$HOME/.local/bin/shell-guardian-fallback"
fi
