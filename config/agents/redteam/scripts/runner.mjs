#!/usr/bin/env node
/**
 * runner.mjs — adversarial code review driven by agent-core provider fallback registries.
 *
 * Usage:
 *   node runner.mjs <lineage|provider/model> [options]
 *   node runner.mjs kimi --repo-root . --focus "auth"
 *
 * Sole adversarial-review runner (the opencode-SDK runner was retired). Drives
 * every lineage over the @moonshot-ai agent-core harness + per-lineage provider
 * fallback registry. opencode-go remains a model PROVIDER in the registry; it is
 * no longer a separate review backend.
 */
...
import { buildReviewEvidencePack, extractVerdictObject, inferLineage, resolveServedModelRef, modelsListUrl, resolveModelsCatalogBaseUrl, shouldWarnAboutHarnessInternals } from "./chain-logic.mjs"

const HERE = dirname(fileURLToPath(import.meta.url))
const PLUGIN = resolve(HERE, "..")
const KIMI_CODE = process.env.KIMI_CODE_ROOT || "/home/mhugo/code/kimi-code"
const REVIEW_REPO_MAP_MAX_CHARS = 12000
...
function fail(summary, code = 2) {
  emit({ verdict: "needs-attention", summary, findings: [], next_steps: [] })
  process.exit(code)
}
function failWith(summary, code = 2, extra = {}) {
  emit({ verdict: "needs-attention", summary, findings: [], next_steps: [], ...extra })
  process.exit(code)
}

// A bare positional `provider/model` runs that lineage's fallback CHAIN. `--model
// <provider/model>` runs that EXACT model directly (bypassing the chain) so arbitrary
// models — incl. small scan-tier ones not in any chain — can be run.
const positionalModel = process.argv[2] && !process.argv[2].startsWith("--") ? process.argv[2] : null
const modelFlag = flag("--model", null)
const pin = modelFlag !== null
const requestedLineage = flag("--lineage", null)
  || (!pin && positionalModel && !positionalModel.includes("/") ? positionalModel : null)
  || (!pin && !positionalModel ? "kimi" : null)
// Bare provider/model is a direct chain head: try that exact provider/model first,
// then continue through the lineage fallback order on transient provider failure.
// --model remains the only exact pin that must not substitute.
const directModel = !pin && positionalModel && positionalModel.includes("/") ? positionalModel : null
const modelArg = modelFlag || positionalModel || requestedLineage
const registryStatic = has("--registry-static")
const slash = modelArg.indexOf("/")
if (slash < 0 && !requestedLineage) fail(`Bad model "${modelArg}" — expected provider/model or --lineage.`)

const providerID = slash < 0 ? requestedLineage : modelArg.slice(0, slash)
...
const focus = flag("--focus", "(none)")
const full = has("--full") // review the ENTIRE repository (explore via tools), not a diff
const targetLabel = flag("--target", full ? "entire repository" : base ? `${base}...HEAD` : "working-tree diff")
const deepwiki = !has("--no-deepwiki")
const timeoutMs = Number.parseInt(flag("--timeout", "600000"), 10)
const explicitMaxTokens = flag("--max-tokens", null)
// null = no explicit cap → the bridge uses each model's own max_output_size
// (flat-rate per-execution billing, so we want the full output budget, never a
// tight truncating cap). An explicit --max-tokens overrides, uncapped.
const maxCompletionTokens = explicitMaxTokens === null ? null : Number.parseInt(explicitMaxTokens, 10)
if (!Number.isFinite(timeoutMs) || timeoutMs <= 0) fail("--timeout must be a positive integer")
if (explicitMaxTokens !== null && (!Number.isFinite(maxCompletionTokens) || maxCompletionTokens <= 0)) fail("--max-tokens must be a positive integer")

// inferLineage now lives in chain-logic.mjs (single home, shared with model discovery).

// In --model (pin) mode the lineage is cosmetic (the exact model runs, not a chain), so a
// model from an unknown family must NOT abort — fall back to a valid placeholder lineage.
const lineage = requestedLineage || inferLineage(modelArg) || (pin ? "deepseek" : null)
if (!lineage) fail(`Could not infer lineage from "${modelArg}" — pass --lineage.`)
...
const PUBLIC_MODES = ["review", "plan", "decision", "security", "ultrareview", "verify", "harvest", "hunt", "smoke", "route", "solve"]
const mode = flag("--mode", "review")
const promptFile = MODE_PROMPTS[mode]
if (!promptFile) fail(`Bad --mode "${mode}" — expected one of: ${PUBLIC_MODES.join(", ")}.`)

function git(args) {
  return execFileSync("git", args, { cwd: repoRoot, encoding: "utf8", maxBuffer: 64 * 1024 * 1024 }).trim()
}

const inputFile = flag("--input", null)
const inputText = flag("--text", null)
let reviewInput = ""
let inputKind = "diff"
if (registryStatic) {
  ...
  try {
    ...
    fail(`--input read failed: ${err?.message ?? err}`)
  }
} else if (inputText) {
  ...
  try {
    // With --base, review base...HEAD (committed range). If that is empty — a common
    // footgun is `--base HEAD`, i.e. HEAD...HEAD = nothing, or a base equal to HEAD with
    // only UNCOMMITTED work — fall back to `git diff <base>` (working tree vs base, which
    // DOES include uncommitted changes), then to the plain working-tree diff and the index.
    // Without --base, review the working-tree diff, then the staged (index) diff.
    if (base) {
...
if (!reviewInput) {
  emit({ verdict: "needs-attention", summary: "No input to review (no diff, no --input/--text); synthetic harness paths cannot approve.", findings: [], next_steps: [] })
  process.exit(0)
}

let evidenceShortstat = ""
let evidenceNameStatus = ""
if (inputKind === "diff") {
  try {
    const diffRange = base ? `${base}...HEAD` : null
    evidenceShortstat = diffRange ? git(["diff", "--shortstat", diffRange]) || git(["diff", "--shortstat", base]) : git(["diff", "--shortstat"]) || git(["diff", "--cached", "--shortstat"])
    evidenceNameStatus = diffRange ? git(["diff", "--name-status", diffRange]) || git(["diff", "--name-status", base]) : git(["diff", "--name-status"]) || git(["diff", "--cached", "--name-status"])
  } catch {
    evidenceShortstat = ""
    evidenceNameStatus = ""
  }
}
function loadReviewRepoMap() {
  const rankedMap = join(repoRoot, ".sf", "repo-map", "REPO_MAP.txt")
  try {
    if (existsSync(rankedMap)) return readFileSync(rankedMap, "utf8")
  } catch {
    return ""
  }
  return ""
}
const evidencePack = buildReviewEvidencePack({
  repoRoot,
  targetLabel,
  inputKind,
  shortstat: evidenceShortstat,
  nameStatus: evidenceNameStatus,
  repoMapText: loadReviewRepoMap(),
  repoMapMaxChars: REVIEW_REPO_MAP_MAX_CHARS,
})
const harnessScopeWarning = shouldWarnAboutHarnessInternals(repoRoot)
  ? "\n\nHarness boundary: `submit_verdict` is the review harness output tool, not part of the target repository. Do NOT search for, inspect, or reason about `submit_verdict`, redteam runner files, or harness schemas unless the target repository under review is the redteam harness itself."
  : ""

const schema = mode === "route"
  ? `{
  "primary": "provider/model",
  "failover": ["provider/model"],
  ...
  "confidence": 0.0,
  "reason": "<one line, cite the code you read>"
}`
  : `{
  "verdict": "approve" | "needs-attention",
  "summary": "<one-line ship/no-ship>",
  "findings": [
    { "severity": "critical|high|medium|low", "title": "...", "body": "...",
      "file": "path/to/file", "line_start": 1, "line_end": 1,
      "confidence": 0.0, "recommendation": "..." }
  ],
  "next_steps": ["..."]
}`

const prompt = registryStatic
  ? ""
  : readFileSync(join(PLUGIN, "prompts", promptFile), "utf8")
    .replaceAll("{{TARGET_LABEL}}", targetLabel)
    .replaceAll("{{USER_FOCUS}}", focus)
    .replaceAll("{{REVIEW_COLLECTION_GUIDANCE}}", deepwiki
      ? "For third-party dependencies or framework behavior, gather evidence in THIS ORDER and stop as soon as you have the answer: (1) READ the dependency's own source — it is usually present under `node_modules` (often a symlink to a local checkout), so this is the cheapest and most accurate; (2) query the DeepWiki MCP tools for the repo's documented behavior; (3) ONLY if local source and DeepWiki do not cover it, use `WebSearch` when available, then `FetchURL` to read a result. Prefer DeepWiki over web search. Never guess about external code. When resolving a repo name, the npm package name is NOT necessarily its GitHub repo — scoped/monorepo packages (an `@scope/pkg` living in a differently-named monorepo) do not map to `scope/pkg`; get the real repo from that package's `repository` field in `node_modules/<pkg>/package.json` rather than inferring it from the import path."
      : "For third-party dependencies or framework behavior, use local source under `node_modules` or repository files first. If that is insufficient, use `WebSearch` when available or `FetchURL` for a known documentation/source URL. DeepWiki is disabled for this run. Never guess about external code. Open every code path you cite with read/grep/glob before asserting it — do not review from the diff alone.")
    .replaceAll("{{SCHEMA}}", schema)
    .replaceAll("{{OUTPUT_INSTRUCTION}}", mode === "smoke" || mode === "route"
      ? "Return ONLY valid JSON matching this schema (no prose, no markdown fences):\n" + schema
      : "Submit your verdict by CALLING the submit_verdict tool. Its parameters define exactly the fields to provide. Do NOT write the JSON in your message — call the tool.")
    .replaceAll("{{REVIEW_INPUT}}", inputKind === "full"
      ? `${evidencePack}${harnessScopeWarning}\n\nFull-repository review (repo: ${repoRoot}). There is NO diff — review the codebase AS IT STANDS. Begin with Glob to map the file tree, then Grep/Read the relevant files, and report findings for the focus area ACROSS the whole repository. Cover it methodically; do not stop at a single file. Prioritise the highest-severity issues you can substantiate by reading the actual code.`
      : inputKind === "document"
        ? `${evidencePack}${harnessScopeWarning}\n\nUnder review (repo: ${repoRoot}): the FULL CONTENT below IS the review subject — there is NO diff or pull request. Review the ENTIRE content as it stands for the issues in your task/focus, reading related files with your tools as needed. Do NOT approve or pass merely because "no change/diff/PR was provided" — the content itself is exactly what you must evaluate.\n\n${reviewInput}`
        : `${evidencePack}${harnessScopeWarning}\n\nDiff under review (target: ${targetLabel}, repo: ${repoRoot}):\n\n\`\`\`diff\n${reviewInput}\n\`\`\``)

// System prompt: reuse agent-core's battle-tested tool-use framing (the same the
// kimi-cli Agent uses) so reasoning models (cogito etc.) issue NATIVE tool calls
// instead of narrating them as text. A thin "you are a reviewer" system prompt
// + our JSON-only output instruction made some models describe tool calls in
// prose and stop, so they never read anything (empty verdicts). The Kimi env
// template vars are stripped; a strong READ-ONLY override is appended.
function buildSystemPrompt() {
  const reviewKind = mode === "plan"
    ? "implementation-plan reviewer"
    : mode === "route"
      ? "model route planner"
    : mode === "decision" || mode === "critique" || mode === "design"
      ? "systems/architecture decision reviewer"
      : "code reviewer"
  const fallback =
    `You are a read-only adversarial ${reviewKind} running inside an agentic tool-use harness.\n\nYou MUST use the provided tools (Read, Grep, Glob) via native tool-calling to gather real evidence — do NOT describe tool calls as text. When calling tools, do not provide explanations. You may issue multiple tool calls in one response. Tool results return in a tool message; determine your next action from them.`
  let framing = ""
  try {
    const sysmd = readFileSync(
      join(KIMI_CODE, "packages", "agent-core", "src", "profile", "default", "system.md"),
      "utf8",
    )
    framing = sysmd.replace(/\{\{[^}]*\}\}/g, "").trim()
  } catch {
    framing = fallback
  }
  if (mode === "route") {
    return "You are a bounded model route planner. Use only the candidate list in the prompt. Return only the requested JSON object."
  }
  return `${framing}\n\n# Review constraints (authoritative)\nThis task is READ-ONLY adversarial ${reviewKind} work. Use the read-only inspection tools currently advertised to you; this prompt is not an exhaustive tool inventory. There are NO write, edit, or bash tools. Never attempt to modify, create, or delete files. Investigate with the tools first (actually CALL them, do not narrate), then submit your verdict by CALLING the submit_verdict tool (its parameters are the required fields); do not write the JSON in prose.`
}
const systemPrompt = buildSystemPrompt()

// Delegate the parse to chain-logic.extractVerdictObject (the SAME extraction the reask
// gate uses) so a result that passes hasVerdict is exactly one that parses here, then add
// the mode-specific constraints (verify: real|false-positive; review: a findings array).
function extractVerdict(text) {
  if (mode === "route") {
    ...
    return { primary, failover, reason: typeof obj.reason === "string" ? obj.reason : "" }
  }
  const obj = extractVerdictObject(text)
  if (!obj) return null
  if (mode === "verify") {
    return obj.verdict === "real" || obj.verdict === "false-positive" ? obj : null
  }
  if (obj.verdict !== "approve" && obj.verdict !== "needs-attention") return null
  if (!Array.isArray(obj.findings)) return null
  return obj
}

const outputSchema = JSON.parse(readFileSync(join(PLUGIN, "schemas", "review-output.schema.json"), "utf8"))
const validateReview = new Ajv2020({ allErrors: true }).compile(outputSchema)
...
// Repair-then-validate, matching opencode's leniency: coerce the cheap field-shape
// mismatches a model commonly emits (numeric fields as strings, missing
// recommendation), then validate. On residual mismatch, return the verdict we
// ALREADY have rather than an empty error verdict — a schema-shape nit must never
// discard real findings (the strict-reject bug).
function normalizeReviewVerdict(verdict) {
  if (mode === "route") return verdict
  if (mode === "verify") return verdict
  if (verdict && Array.isArray(verdict.findings)) {
    for (const f of verdict.findings) {
      if (f && typeof f === "object") {
        if (f.confidence !== undefined) f.confidence = toNumber(f.confidence)
        if (f.line_start !== undefined) f.line_start = toNumber(f.line_start)
        if (f.line_end !== undefined) f.line_end = toNumber(f.line_end)
        if (f.recommendation === undefined || f.recommendation === null) f.recommendation = ""
      }
    }
  }
  if (verdict && !Array.isArray(verdict.next_steps)) verdict.next_steps = []
  if (validateReview(verdict)) return verdict
  // Validation still failed on shape — keep the findings, don't nuke them. Annotate
  // the verdict so the schema drift is visible without losing the substance.
  verdict.schema_warning = validateReview.errors?.map((e) => `${e.instancePath || "/"} ${e.message}`).join("; ")
  return verdict
}
...
const REASK = mode === "verify"
  ? 'Output ONLY the JSON verdict object: {"verdict":"real"|"false-positive","confidence":0.0,"reason":"..."}. No prose, no fences, no comments.'
  : 'Call the submit_verdict tool now to complete your review. Pass your verdict ("approve" or "needs-attention"), summary, findings array, and next_steps as the tool arguments. Do NOT write JSON in prose — call the tool.'

async function runBridge() {
  const BUNDLE = join(HERE, "bridge.bundle.mjs")
  const bridgeSources = [
    "runner.mjs",
    "chain-logic.mjs",
    "provider-gate.mjs",
    "deepseek-tool-parser.mjs",
  ].map((name) => join(HERE, name))
  const bundleStale = (() => {
    if (!existsSync(BUNDLE)) return true
    try {
      const bundleMtime = statSync(BUNDLE).mtimeMs
      return bridgeSources.some((source) => statSync(source).mtimeMs > bundleMtime)
    } catch {
      return true
    }
  })()
  if (bundleStale) {
    // The bundle is gitignored (rebuilt from the BRIDGE template by build-bridge).
    // Self-heal a fresh or stale checkout rather than executing old routing code.
    execFileSync(process.execPath, [join(HERE, "build-bridge.mjs")], { cwd: PLUGIN, stdio: "inherit" })
  }
  const tmpRoot = join(PLUGIN, ".tmp")
  mkdirSync(tmpRoot, { recursive: true })
  const tmp = mkdtempSync(join(tmpRoot, "runner-"))
  try {
    const inputPath = join(tmp, "input.json")
    writeFileSync(inputPath, JSON.stringify({ prompt, systemPrompt, reask: REASK, repoRoot, providerID, modelID, lineage, pin, directModel, registryStatic, mode, deepwiki, timeoutMs, maxCompletionTokens }))
    // Async spawn (NOT spawnSync): spawnSync buffers all output and only returns it
    // when the bridge exits, so per-step progress could never stream. Relay the
    // bridge's stderr live so a parent (the panel, a background task) sees progress
    // as it happens. Keep a hard kill backstop matching the old spawnSync timeout.
    const MAX_BUFFER = 64 * 1024 * 1024 // parity with the old spawnSync maxBuffer
    const child = await new Promise((resolve, reject) => {
      const c = spawn(process.execPath, [BUNDLE], {
        ...
        env: {
          ...
          REDTEAM_INPUT_FILE: inputPath,
        },
      })
      // setEncoding attaches a StringDecoder so multi-byte UTF-8 (CJK, emoji in
      // model output) is not corrupted at chunk boundaries — the old spawnSync got
      // this for free via encoding: "utf8".
      c.stdout.setEncoding("utf8")
      c.stderr.setEncoding("utf8")
      let stdout = ""
      let stderr = ""
      let overflow = false
      const hardKill = () => { try { c.kill("SIGKILL") } catch {} }
      // Backstop: SIGTERM first so the bridge's finally blocks run (release the
      // provider-gate slot), then SIGKILL if it ignores the grace period.
      const killer = setTimeout(() => {
        try { c.kill("SIGTERM") } catch {}
        const g = setTimeout(hardKill, 5000); g.unref?.()
      }, timeoutMs + 30000)
      // Bound accumulation so a runaway bridge can't OOM the parent (the old
      // spawnSync enforced this via maxBuffer). Kill + reject on overflow.
      const overran = (len) => {
        if (len <= MAX_BUFFER) return false
        overflow = true
        clearTimeout(killer)
        hardKill()
        reject(new Error(`bridge output exceeded ${MAX_BUFFER} bytes`))
        return true
      }
      c.stdout.on("data", (d) => { if (!overflow && !overran(stdout.length + d.length)) stdout += d })
      c.stderr.on("data", (d) => { if (!overflow && !overran(stderr.length + d.length)) { stderr += d; process.stderr.write(d) } })
      c.on("error", (e) => { clearTimeout(killer); reject(e) })
      c.on("close", (code, signal) => { clearTimeout(killer); if (!overflow) resolve({ status: code, signal, stdout, stderr }) })
    })
    if (child.status !== 0) {
      // Prefer a STRUCTURED _bridgeError from stdout (the clean chain-exhaustion message)
      // over raw stderr, which carries streamed step-progress and — for an uncaught crash —
      // a Node banner. Falls back to stderr/stdout for a genuine (non-structured) crash.
      let structured = null
      let structuredObj = null
      try {
        const last = child.stdout.trim().split("\n").filter(Boolean).at(-1)
        const o = last ? JSON.parse(last) : null
        if (o && typeof o._bridgeError === "string") {
          structured = o._bridgeError
          structuredObj = o
        }
      } catch { /* not structured — fall through to raw output below */ }
      const e = new Error(structured || (child.stderr || child.stdout || `bridge exited code=${child.status} signal=${child.signal || "none"}`).trim())
      if (Array.isArray(structuredObj?.providerFailures)) e.providerFailures = structuredObj.providerFailures
      throw e
    }
...
try {
  // No outer gate: the bridge gates each attempt on its real serving provider.
  ...
  if (!verdict) {
    fail(`Moonshot reviewer did not return parseable verdict JSON. Raw output: ${String(text).slice(0, 1000)}`, 1)
  }
  const normalized = normalizeReviewVerdict(verdict)
  // Attribute the served hop (chain may substitute a different provider/model than requested).
  if (normalized && typeof normalized === "object") {
    normalized.model = resolveServedModelRef(bridgeResult, modelArg)
    normalized.requested = modelArg
    if (Array.isArray(bridgeResult.providerFailures)) normalized.providerFailures = bridgeResult.providerFailures
  }
  emit(normalized)
} catch (err) {
  failWith(`Lineage review failed: ${err?.message ?? err}`, 1, {
    providerFailures: Array.isArray(err?.providerFailures) ? err.providerFailures : [],
  })
}
