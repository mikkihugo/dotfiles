...
Pi discovers skills from configured skill directories and installed Pi packages. A shared skills package should expose `skills/` through its `pi.skills` manifest entry. Pi does not expose Claude Code's `Skill` tool, but the agent should still follow the shared skill rule: when a skill applies, load and follow it before responding.
