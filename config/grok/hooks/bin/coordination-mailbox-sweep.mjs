...
export class CoordinationBus {
  ...
  async sweep(consumer, signal) {
    ...
    // Same persist as _doSubscribe. Sweep is the live Grok path; subscribe
    // alone used to mint inbox_uri and this method dropped it, so the next
    // hook/agent sweep was refused as "different principal" (#452).
    const minted = typeof result?.inbox_uri === "string" && result.inbox_uri
      ? result.inbox_uri
      : undefined;
    if (minted) {
      this._inbox = {
        ...
        principal: this._principal,
        session: this._session,
        sequence: Number.isInteger(result?.ack_watermark)
          ? result.ack_watermark
          : this._inbox?.sequence,
        issued_at: new Date().toISOString(),
      };
...
/**
 * Actionable setup hint printed after every identity-derivation failure.
 * Operators reading the error in a session-start TUI line should be able to
 * fix the wiring without opening the source. Three independent paths cover
 * the supported hook wirings:
 *
 *   1. explicit env override (`REPO_MEMORY_SWARM_CONSUMER`) -- universal
 *      fallback used by every client wrapper and the opencode plugin;
 *   2. payload field (`session_id` / `thread_id` / `conversation_id`) -- the
 *      codex/claude/copilot hook harnesses forward these as JSON on stdin;
 *   3. client-specific env var (`CODEX_THREAD_ID`, `CURSOR_CONVERSATION_ID`,
 *      `JCODE_HOOK_SESSION_ID`) -- belt-and-braces fallback for clients
 *      that don't emit a payload-derived id under the unified field names.
 *
 * The hint is appended to every throw below rather than only the catch in
 * `main()`, because the throw site is what an operator pasting the error
 * message back at the agent sees. `COORDINATION_MAILBOX_DEBUG=1` is the
 * cheapest verification step and is named explicitly.
 */
function identitySetupHint(client) {
  const safeClient = String(client ?? "").trim() || "your-client";
  const exampleAssignment = `REPO_MEMORY_SWARM_CONSUMER='${safeClient}-<token>'`;
  return [
    ``,
    `How to set the coordination-mailbox identity for "${safeClient}":`,
    `  1. Easiest (works for every client, including opencode): export`,
    `     ${exampleAssignment}    (4-16 alphanumeric chars, e.g. "abcd1234").`,
    `     Example:  export REPO_MEMORY_SWARM_CONSUMER='${safeClient}-abcd1234'`,
    `  2. Or include a session id in the hook payload (JSON on stdin):`,
    `     {"session_id":"<id>"} | {"thread_id":"<id>"} | {"conversation_id":"<id>"}.`,
    `  3. Client-specific env vars (used when payload is empty):`,
    `     codex/code: CODEX_THREAD_ID   cursor: CURSOR_CONVERSATION_ID`,
    `     jcode: JCODE_HOOK_SESSION_ID`,
    `  4. opencode's coordination-mailbox-sweep.js plugin does NOT forward a`,
    `     payload to the .mjs (it shells out with "< /dev/null"), so option 1`,
    `     is the only way to wire opencode today.`,
    `Verify with: COORDINATION_MAILBOX_DEBUG=1 <hook invocation>`,
  ].join("\n");
}
...
export function validateIdentity(identity) {
  ...
  if (!trimmed) {
    throw new Error(`coordination-mailbox identity is empty.${identitySetupHint("(unknown client)")}`);
  }
  if (BARE_CLIENT_NAMES.has(trimmed)) {
    throw new Error(
      `coordination-mailbox identity "${trimmed}" is a bare client name; use <client>-<short-session-id>.${identitySetupHint("(unknown client)")}`,
    );
  }
  ...
  if (!/^[a-z][a-z0-9]*-[a-z0-9]{4,16}$/.test(trimmed)) {
    throw new Error(
      `coordination-mailbox identity "${trimmed}" must be <client>-<token> with a dash-free client and a 4-16 character alphanumeric token (downstream principal pattern).${identitySetupHint("(unknown client)")}`,
    );
  }
  return trimmed;
}
...
export function deriveIdentity(client, payload, env = process.env) {
  ...
  if (!raw) {
    throw new Error(`missing session-unique coordination-mailbox identity for ${client}.${identitySetupHint(client)}`);
  }
